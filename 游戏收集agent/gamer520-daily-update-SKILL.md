...
- 网站的列表页上有游戏条目的标题，更新日期等信息，然而标题是编辑提交的并不一定准确，详情页的各种信息都更加准确，但需要点击获取，外部搜索/查看steam相关内容等外部来源可以得到更加可靠的信息，但获取的代价就更大了。如果没有必要，不必额外获取太多信息，通常详情页信息就已经足够。
...
- **输出摘要**：简短说明新增/跳过数量、总条目数、最值得注意的几款、推荐原因。可以推荐分数较高（比较符合口味的），也可以即使整体分数不高，但是有独特亮点，或者某个点很合口味的。推荐游戏时必须附链接。
...
- 单条修改用 `update` 命令，无需手动编辑 CSV。PC/Switch 合并暂不进入 CLI。
...
# 修改已有条目的字段（如更新用户备注）
uv run --project 游戏收集agent/gamer520_cli gamer520 update --title "舒适森林 Cozy Grove" --set "用户备注=玩过，已收录到有意思中。" --dry-run
uv run --project 游戏收集agent/gamer520_cli gamer520 update --title "舒适森林 Cozy Grove" --set "用户备注=玩过，已收录到有意思中。" --yes

# 多字段同时修改
uv run --project 游戏收集agent/gamer520_cli gamer520 update --title "游戏名" --set "推荐度=4" --set "推荐标签=推荐" --dry-run
```
